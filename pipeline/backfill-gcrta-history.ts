/**
 * One-off backfill: process 6 historical GCRTA GTFS zips in chronological order,
 * write history snapshots to atlas-archive, and archive each zip.
 * Does NOT touch atlas/gcrta.json (current live data).
 *
 * Run: npx tsx pipeline/backfill-gcrta-history.ts
 */
...
const FEEDS = [
  '/Users/ryan/Downloads/gcrta Jun 27 2014.zip',
  '/Users/ryan/Downloads/gcrta 2016.zip',
  '/Users/ryan/Downloads/gcrta jul 10 2018.zip',
  '/Users/ryan/Downloads/gcrta Jun 16 2020.zip',
  '/Users/ryan/Downloads/gcrta june 2 2022.zip',
  '/Users/ryan/Downloads/gcrta Jun 02 2024.zip',
];
...
async function main() {
  ...
  for (const path of FEEDS) {
    ...
    const periodKey = feedExpiry ?? feedVersion ?? path.replace(/.*gcrta\s*/i, '').replace('.zip', '').trim().replace(/\s+/g, '-');
