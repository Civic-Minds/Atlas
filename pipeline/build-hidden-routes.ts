...
interface Agency { slug: string; name: string; region?: string | null; staged?: boolean; hiddenInProduction?: boolean }
...
await runWithConcurrency(tasks.map(task => async () => {
  try { await task(); } catch (error) { failures++; console.warn(`  [warn] agency artifact failed: ${error instanceof Error ? error.message : error}`); }
}), 12);
const inventory = mergeHiddenRoutes(null, results.map(({ agency, routes }) => ({ agencySlug: agency.slug, routes })));
