...
exports.calculateAgencyHealth = calculateAgencyHealth;
const db_1 = require("../storage/db");
const logger_1 = require("../logger");
async function calculateAgencyHealth(agencyId) {
    const pool = (0, db_1.getPool)();
    logger_1.log.info('Health', 'Calculating feed health', { agencyId });
    // 1. Position Plausibility (% high confidence matches in last 5m)
    const confidenceRows = await pool.query(`SELECT 
       COUNT(*) as total,
       COUNT(CASE WHEN match_confidence >= 0.7 THEN 1 END) as plausible
     FROM vehicle_positions
     WHERE agency_id = $1 AND observed_at >= NOW() - INTERVAL '5 minutes'`, [agencyId]);
    ...
    const assignmentRows = await pool.query(`SELECT 
       COUNT(*) as total,
    ...
    const stabilityRows = await pool.query(`WITH snapshots AS (
    ...
    const compositeScore = Math.round((positionPlausibility * 0.4) +
