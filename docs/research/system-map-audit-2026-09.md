...
The current sample contains the first 100 agencies in catalog order. Records 1–50 are the first batch; records 51–100 are the second batch. Each record begins as `pending_map_review` and must be updated only after the official system map or an approved official rider-facing definition is reviewed.
