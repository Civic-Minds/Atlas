...
#### `maxGapByPeriod` — longest wait represented inside each period (#281)

This additive field finds the largest departure gap that overlaps a period and clips it to the period window. A gap from 8:00 to 10:00 contributes 60 minutes to Midday, so a gap crossing a boundary is not silently dropped from both periods. The field is paired with `headwayByPeriodSustained` and is shown on route cards when the median is not representative.

#### `headwayByPeriodSustained` — a parallel diagnostic flag (#281)
