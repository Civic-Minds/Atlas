...
**Status:** ~48 French agencies are now in the registry as candidates (`hiddenInProduction` + `pmtilesPending`), all dry-run validated with route geometry — including Lyon, Marseille, Toulouse, Nice, and a wide mid-size city sweep (2026-07-19). Still **zero** production-visible French agencies; nothing launched to live R2 without an explicit country-launch decision. Blocked (no usable `shapes.txt` / stops-only): Montpellier, Lille, Strasbourg, and TeMo'b (Thionville). The plan remains: launch France once coverage is broad enough, not city-by-city.
