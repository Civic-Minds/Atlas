...
Registry of agencies targeted for Atlas NextGen live data collection. Route filters and API key status documented here.

**Status as of June 2026:** The OCI server that hosted active polling was decommissioned. No agencies are currently being polled. This registry reflects the planned configuration for when the live data layer is rebuilt on the new stack (Cloudflare Workers + Neon Postgres).

---

## Tier 1 — Full Network (Beta Validation Targets)

Full GTFS-RT polling (no route filter) + static GTFS + stop_times. These are the agencies the trip-matching pipeline should be validated against first.

| Agency | ID | Region | Static GTFS | Notes |
|--------|----|--------|-------------|-------|
| Halifax Transit | `halifax` | Nova Scotia, CA | ✓ (last imported 2026-04-20) | RT trip IDs differ from static — time-based spatial fallback handles matching. Route 320 (airport) confirmed in static GTFS. |
...
Filtered to key routes. Static GTFS and stop_times loaded. Trip matching active but IDs may mismatch (time-based fallback applies).

| Agency | ID | Region | Routes | Notes |
|--------|----|--------|--------|-------|
| Toronto Transit Commission | `ttc` | Ontario, CA | 501, 504–512 (streetcars) + Line 6 Finch West LRT | Clever Devices RT trip IDs don't match Toronto Open Data static IDs — ~20% match rate via spatial fallback. |

---

## Tier 3 — Partial Network, Data Collection Only

Filtered to key routes. No static GTFS — delay_seconds will be null until static is imported. Collecting raw vehicle positions for future analysis.
...
- **MetroTransit** — Persistent HTTP 404 on `vehiclepositions.pb`. Feed URL may have changed — verify against current developer portal before enabling.
...
- **STA** — Intermittent protobuf buffer overflow from Spokane's server returning malformed/truncated responses. Matches 100% when feed is healthy.
- **SDMTS** — Intermittent `fetch failed`. Likely network-level instability on their end.

---

## Keys In Hand — Not Yet Activated
...
- **LA Metro Rail** (`lametro`) — A Line (801), E Line (804). Swiftly key obtained — covers LA Metro only, not Miami/Vegas.
...
- **Edmonton LRT**: Capital/Metro/Valley Line West absent from ETS vehicle positions feed. May be on a separate feed. Route IDs are zero-padded (route 4 = `004`).
