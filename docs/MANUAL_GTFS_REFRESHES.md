...
expired production snapshots: 36 now have newer sources, 59 remain genuinely
...
The 59 agencies below remain candidates for source recovery. The read-only audit
...
The following 36 agencies have a current ZIP with matching agency identity and
can be refreshed using the configured or automatically derived source:

`abqride`, `carta-chattanooga`, `eugene-ltd`, `gold-coast`, `goldengate`, `goraleigh`, `mont-tremblant`,
