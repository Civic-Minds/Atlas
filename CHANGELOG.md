...
- **History: reuse standard AgencyCard component**: Completely deleted the duplicate `AgencyView` component and custom route list rendering logic in `History.tsx`. Instead, lifted the `day` and map `layers` state up to `App.tsx` and reused the exact same standard `AgencyCard` component in both Frequency and History apps. This guarantees 100% design parity and eliminates duplicate styling code.
