...
- **History: Route History Card & Agency Profile Card**: Redesigned the History app drill-down flow. Selecting an agency opens an Agency Profile Card containing an agency details widget (name, region, routes count) styled identically to the Route Card, with a structured sub-list of routes inside a rounded border container. Clicking a route opens a dedicated `RouteHistoryCard` for the timeline snapshots and headway changes, which highlights the selected route line on the map.
