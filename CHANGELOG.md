...
- **History: styled route cards in AgencyView**: Redesigned the route list within the history agency profile view to render as a list of distinct, always-expanded Route Cards. Each route gets its own card with a subtle background, border, and shadow. This separates routes cleanly and displays the full snapshot timeline directly inside the card where the years (e.g., `2008 Launch`, `2016`, `2026`) are clearly visible under each headway number, providing instant chronological context without visual clutter.
