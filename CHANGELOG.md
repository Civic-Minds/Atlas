...
- **History: Route History Card view**: Redesigned the History app drill-down flow to render a clean, compact route list. Clicking a route opens a dedicated `RouteHistoryCard` containing the full snapshots timeline and frequency change summary statements, utilizing the shared `FLOATING_CARD` design component. Also feeds the selected route to the map overlay to highlight the selected route line on the map in context.
