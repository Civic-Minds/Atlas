...
- **History: Route History Card & Agency Profile Card**: Redesigned the History app drill-down flow to match the visual architecture of the Frequency screen's route cards. Custom header layouts (large title, region/route count subtitle on left, back arrow circle button on right, no border dividers) are placed directly on the main card canvas, and nested inner grey boxes are removed so route details and lists render directly on the card face.
