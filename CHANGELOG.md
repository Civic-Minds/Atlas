...
### Fixed
- **Time scrubber design consistency**: The History time scrubber now uses the app's FLOATING_CARD surface (rounded-2xl + proper bg/border/shadow, full width of container with no mx inset) instead of plain narrower container. Range input fully restyled: thin rounded-full track (9999px), 16px fully rounded thumb (50% radius) with accent border + panel bg + subtle shadow (matching other app accents, dots, and controls). Header and labels use consistent text-[10px]/text-[9px] sizes (no forced uppercase or extra-small 7-8px, no font-mono on labels), matching other panel headers and dim text exactly. Only mono on current year if needed. Colors pulled from CSS vars. (Previously mismatched width and fonts vs. other FLOATING_CARD panels, "Recent searches", etc.)
