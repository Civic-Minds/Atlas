...
### Fixed
- **Route card direction groups (#97)**: Multi-direction routes show Westbound/Eastbound (etc.) section labels, a divider between groups, and west-to-east sort from branch terminal geometry. Bearing uses mean shape start→end so branched routes label correctly.
