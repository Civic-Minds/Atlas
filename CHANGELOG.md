...
- **History: accordion route list in AgencyView**: Redesigned the route list within the history agency profile view to use an accordion layout. Routes are listed compactly with a frequency change preview (e.g. `5m → 15m`), and clicking a route expands to show its detailed snapshot timeline, preventing massive visual clutter.
