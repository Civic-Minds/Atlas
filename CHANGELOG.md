...
- **Fixed routes showing a stuck/stale frequency at their terminus** (#263): a sparse branch's own headway (computed from however few trips run that day, with no minimum sample size) could get ratcheted in over a more accurate, better terminal-specific number and never get corrected — e.g. a Rennes Sunday route showing "every 121 min" when every stop, including the terminus, actually ran every 60 min. The protection this ratchet exists for (a shared terminal's combined frequency shouldn't falsely improve a branch's real number, confirmed still working on TTC route 35) now only applies when the branch's own number is backed by enough trips to trust in the first place. Confirmed across Rennes and Lyon during this fix; likely corrects similar cases on other low-frequency routes systemwide on the next refresh.
...
- **Vetted the France batch's real-processed staging data (Lyon, Metz, Nice, Rennes, Toulouse) against `route-report`'s 36 flags for the first time**: ~26 confirmed legitimate trunk/branch short-turn patterns (verified via opposite-direction sanity checks and per-stop breakpoint analysis). Fixed 2 real headsign bugs: Lyon ZI8's un-accented "Parc des Lumieres" variant carried no real trip data (0 stops, an orphaned duplicate of "Parc des Lumières 1"), and Nice 69's "Leï Feirriero"/"Lei Feirrièro" split the same destination across two inconsistent accent transcriptions in the source feed. Both now merge into one canonical headsign. Remaining ~7 flags are lower-confidence and still need a closer look, including a signature (flat per-stop headway contradicted by a higher reported terminal headway) seen on infrequent-tier routes in multiple cities — that one traces to a pre-existing pipeline methodology question, not a France-specific issue, and needs its own investigation before any fix.
...
- Two more Nancy Réseau Stan shapes (routes T2 and Corol) had a single misplaced point creating an isolated reversal the general detector can't safely catch — a broader heuristic was tried and rejected after it came within ~3 points of misfiring on a real TTC terminus loop, so these two specific shapes are fixed by an explicit, narrowly-scoped exception instead of a general rule.
