...
- **History app: filter map to history agencies only**: in History mode the map now only renders route layers for agencies with actual history data. Non-history agencies (e.g. Calgary) no longer appear on the map when browsing History. Falls back to all agencies until history-config loads.
