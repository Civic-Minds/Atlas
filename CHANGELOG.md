...
- **Map routes now filter correctly for agency search**: Searching an agency (slug like "ttc"/"hamilton" or name) in the main bar was updating sidebar counts/cards (via useIntervalStats) but the MapCanvas vector filter only did `like` on routeShortName/routeId. This resulted in zero matches on the map, so "X routes" would appear but no lines were visible. Now builds exact `agencySlug ==` filter for the map when q matches an agency (mirrors the stats logic). Route number searches continue to use like.
