...
- **History: reuse standard AgencyCard component**: Completely deleted the duplicate `AgencyView` component and custom route list rendering logic in `History.tsx`. Lifted the `day` and map `layers` state up to `App.tsx` and reused the exact same standard `AgencyCard` component in both Frequency and History apps. To fix absolute nesting alignment offsets, refactored the History outer wrapper container to `absolute inset-0 pointer-events-none` with inner `pointer-events-auto`, aligning all cards perfectly under the search bar.
