...
### Fixed
- **Time scrubber design consistency**: The History time scrubber now uses the app's FLOATING_CARD surface (rounded-2xl + proper bg/border/shadow) instead of plain container. Range input fully restyled: thin rounded-full track (9999px), 16px fully rounded thumb (50% radius) with accent border + panel bg + subtle shadow (matching other app accents, dots, and controls). Labels use consistent 8-9px sizes, tracking-wider uppercase, font-mono for years. Colors pulled from CSS vars (--accent, --bg-panel, --border-primary, --text-muted) for light/dark parity. (Previously had sharp/default browser range appearance and mismatched tiny text.)
