...
- **Stale schedule help**: Route card "Learn more" opens in-app outdated-schedule explainer instead of GitHub docs; help panel shows when Atlas last fetched the feed (`lastRefreshedAt`, backfilled from last weekly refresh for existing agencies). Expired-feed agencies get `websiteUrl` from GTFS with a "Check current schedules" link in the help panel. Next refresh countdown reads `public/data/feed-refresh.json` (cron synced from `refresh-feeds.yml`, `lastCompletedAt` stamped by the Action). About panel footer uses the same refresh metadata and honest GTFS/MDB source wording.
