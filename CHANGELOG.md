...
- Shared-core frequencies now appear beside branch frequencies, and qualifying cores no longer show a misleading outside-filter warning.
...
- Fixed the night-service check missing real overnight-only service recorded under its own separate service_id instead of the day's main schedule ([#297](https://github.com/Civic-Minds/Atlas/issues/297))
- Fixed some sparse route variants never getting a night-service value at all instead of correctly showing no night service ([#301](https://github.com/Civic-Minds/Atlas/issues/301))
...
- **Live snapshot/replay errors no longer leak infrastructure detail**: clients get a generic unavailable message instead of raw R2/AWS error text.
...
- **France expansion grew to ~142 dry-run candidate cities across four batches this cycle**: major metros (Lyon, Marseille, Toulouse, Nice, Cannes, Limoges, …) and a wide mid-size sweep across every métropole région, then two more speedrun batches (Cherbourg, Chamonix, Albi, Angoulême, Montauban, Arcachon, … and Bourg-en-Bresse area, Narbonne, Sète, Haguenau, La Roche-sur-Yon, Dreux, …). Every feed dry-run validated with route LineStrings near the claimed city; still fully `hiddenInProduction` / `pmtilesPending` until a deliberate country launch — zero R2 writes.
- **Fixed agency-name display collisions from the France batch**: Citibus (Lubbock/Narbonne), TAC (Annemasse/Charleville-Mézières), and TUB (Saint-Brieuc/Bergerac) are genuine same-brand coincidences (verified against each network's real name), not a bug — added to the documented shared-acronym allow-list alongside CAT/GET/TCAT/etc.
- **Fixed a real slug collision that had silently overwritten a live agency's config**: the France batch's "Nice" candidate was written to `config/agencies/nice.json`, the same slug already used by Nassau Inter-County Express (NICE Bus, Long Island NY) — a real, already-live agency, whose config was fully replaced in-place. Production R2 data was unaffected (only the git-side config changed), but merging as-is would have deleted a live agency on the next index rebuild. Restored NICE Bus's config and moved the French candidate to a new `nice-fr` slug, matching the existing `springfield-mo`-style disambiguation convention. Checked all 142 France candidate slugs against the full existing registry — this was the only collision.
- **`route-report --live` was overcounting its own flag total**: leftover local dry-run shape-anomaly files inflated the printed total even though `--live` mode already correctly hides that category's detail (it's dry-run-only data). Total now matches what's actually shown.
- **Vetted the France batch's real-processed staging data (Lyon, Metz, Nice, Rennes, Toulouse) against `route-report`'s 36 flags for the first time**: ~26 confirmed legitimate trunk/branch short-turn patterns (verified via opposite-direction sanity checks and per-stop breakpoint analysis). Fixed 2 real headsign bugs: Lyon ZI8's un-accented "Parc des Lumieres" variant carried no real trip data (0 stops, an orphaned duplicate of "Parc des Lumières 1"), and Nice 69's "Leï Feirriero"/"Lei Feirrièro" split the same destination across two inconsistent accent transcriptions in the source feed. Both now merge into one canonical headsign. A recurring signature among the remaining flags (flat per-stop headway contradicted by a higher reported terminal headway) turned out to be issue #263, fixed separately above.
- **Route titles with 3+ termini no longer cut off mid-word**: STAR (Rennes) route 12's title overflowed its 2-line clamp and got cut off as "...↔ Rennes (La..." — a route with a middle via-point (e.g. "A ↔ Via-point ↔ B") now shows just the through-route's first and last terminus in the title, matching how it's shown everywhere else the route is listed. The full stop-by-stop path is still on the map itself ([#250](https://github.com/Civic-Minds/Atlas/issues/250)).
- **Pipeline hard-blocks accidental R2 publishes for unlaunched countries**: `npm run process`, `npm run build-pmtiles-incremental`, and `npm run refresh` now refuse (or skip, for batch refresh) live writes when the target country still has zero production-visible agencies — France and Mexico today. Override only with explicit `--i-am-launching-country` after maintainer approval. Stops the "forgot `--dry-run` / cited Metz as precedent" failure mode that already put Metz on R2 unapproved.
- **Route cards no longer drop a direction that has real service**: STAR (Rennes) route 77 showed only "to Pacé" with no return-direction row, despite 560 real weekday trips running the other way. The short-turn exclusion logic used raw shape point count as a proxy for a shape's real length, and separately assumed a shorter shape always means a rarer one — both broke down here: a rarely-used 18-trip detour shape was both the most densely-digitized (561 points) and, by real geographic distance, genuinely the longest, so it won as "the" analysis shape while excluding the shape 97% of trips actually used. Now measures real distance instead of point count, and falls back to trip volume whenever the length-based pick covers less than half of all trips ([#249](https://github.com/Civic-Minds/Atlas/issues/249)).
- **"Near You" panel no longer overlaps the zoom/geolocate controls**: it sat at a fixed 72px from the bottom, which only cleared the geolocate button and actually overlapped the stacked zoom in/out buttons by about half their height. Repositioned above the whole control cluster.
- **Niagara Transit routes 104/105/109 no longer show a mismatched-color line segment**: each route's map line showed part of its length in a much-higher-frequency tier color than its own card (e.g. route 105's card said "every 60 min" for both directions, but the line rendered a segment in the ≤15 min color, in one case as a fragment fully disconnected from the rest of the route). Root cause was the same stale-data pattern as GO Transit below — `minStopHeadway` was frozen at 2 min in production from before an existing branch/shared-stop fix landed, and the map's line-color expression trusts `minStopHeadway` over the route's own headway. Forced a fresh reprocess; no code change needed ([#252](https://github.com/Civic-Minds/Atlas/issues/252)).
- **GO Transit 41 headways corrected to real clockface intervals**: "to Square One" showed a blended 17 min against a real 30, and "to Centennial College" showed 13 against irregular actual service — production data was stale, generated before an existing pipeline fix for GO's per-calendar-date service_id pattern had been applied to this agency. Forced a fresh reprocess; no code change needed ([#251](https://github.com/Civic-Minds/Atlas/issues/251)).
- **Fixed a class of doubled-frequency headway bugs from overlapping calendar.txt periods**: TAG (Grenoble) Tram A showed "every 5 min" when the real service runs every 10 — confirmed against raw trip/stop data that two overlapping weekday `calendar.txt` service periods were both being counted, even though `calendar_dates.txt` explicitly cancels one of them (exception_type 2) for nearly every real date in the overlap. The pipeline only ever applied `calendar_dates.txt` overrides to services with no `calendar.txt` entry at all, so a cancelled-but-still-calendar.txt-listed service kept merging its trips into the count, doubling apparent departures. Sequential schedule exports with this exact overlapping-then-superseded pattern are common in French feeds, so this likely affected other agencies too, not just Grenoble.
- **French headsigns starting with "Le" no longer get shouted**: TBM (Bordeaux) Tram A showed "to LE Haillan Rostand" — GO Transit's 2-char line codes (LW/LE/KI/MI/BR) were force-uppercased anywhere they appeared as a whole word, which collided with the French article "Le" in place names (Le Haillan, and likely other "Le ..." places on French feeds generally). The line codes are still uppercased correctly as a standalone route label ("LW — Lakeshore West"); only the mid-sentence substitution — which no real GO Transit name actually needed — was removed.
...
- **Fixed a second false-positive class, found by validating the detector against live Canada/US agencies (TTC, WMATA, TransLink) before trusting it broadly**: a near-duplicate point (two points ~10cm apart, common where a feed snaps a shape point to a stop location) makes the incoming bearing calculation meaningless, misread as a sharp reversal. Now requires the incoming segment to be at least 2m before trusting its bearing.
- **Fixed 5 more Nancy Réseau Stan isolated-point shape corruptions** (routes T2 and Corol variants), found via a full sweep for the same single-misplaced-point pattern the general detector can't safely catch — kept as an explicit, narrowly-scoped exception list rather than a general rule, since a broader heuristic came within ~3 points of misfiring on a real TTC terminus loop. One of these (STAN-67$16) needed two adjacent points removed, not one, to fully close the visible gap. Four other sweep candidates matched the noise signature instead and were left alone.
- **Right-click the map for a small menu**: copy a URL pointing at that exact spot, or jump straight to filing a GitHub issue pre-filled with the location and tagged `user-reported` — both skip having to recenter the map and manually copy coordinates first.
...
- **A direct link to a specific agency's route/stop now defaults to that agency's own current time of day, not the viewer's browser clock**: opening a UK or Mexico agency from Toronto at 11pm local no longer lands on "late/overnight" just because it's late where the viewer happens to be. Requires the agency's timezone, newly captured from GTFS `agency.txt` during processing — existing agencies fall back to the old (local-clock) behavior until they're next reprocessed.
...
- **Incremental PMTiles build**: Added `npm run build-pmtiles-incremental -- <slug> [--dry-run]` to add a single new, geographically isolated agency (e.g. Metz, France) into the deployed `atlas.pmtiles` without a full ~467-agency rebuild. Refuses with a clear error if the agency is already published or its bbox overlaps any existing agency (would corrupt the stops layer's density-based tile decisions) — see `docs/ADDING_AGENCIES.md` § Incremental PMTiles Build.
- **Mi Transporte (Guadalajara) T14B_r1: fixed the deeper shape corruption**: unlike T14B_r2's single bad coordinate jump (already fixed), T14B_r1 had two entire physical paths concatenated under one `shape_id` with colliding, duplicated `shape_pt_sequence` numbers — sorting by sequence alone interleaved both paths into a zigzag. Duplicate sequence numbers now resolve by keeping whichever candidate point is spatially nearest the previously-selected point, so the shape stays on one continuous path. Both T14B shapes now correctly resolve to the same physical corridor traversed in opposite directions ([#244](https://github.com/Civic-Minds/Atlas/issues/244)).
...
- **Restored missing YRT trip-lookup data**: `atlas/yrt-trips.json` had silently failed to publish during a past refresh, so York Region Transit's live vehicles had no direction/destination data to fall back on. Re-published via a forced refresh ([#230](https://github.com/Civic-Minds/Atlas/issues/230)).
- **Mexico coverage hidden from production until validated**: Mi Transporte (Guadalajara) was visible to real users while known data-quality issues in its coverage are still being fixed. Now hidden from the public build (and the external agency directory other tools consume) while remaining visible in local development for QA ([#222](https://github.com/Civic-Minds/Atlas/issues/222)).
- **Corrupted shapes.txt jumps no longer render as straight lines across the map**: a shape with one badly corrupted coordinate pair — confirmed on Mi Transporte (Guadalajara) route T14B, a ~16.7km jump on an otherwise ~50m-spaced shape — used to draw a long straight artifact across unrelated parts of the city. Shapes are now truncated at the first such implausible, unambiguous jump rather than rendered whole (#219 partial fix — a second, more deeply corrupted T14B shape variant with interleaved duplicate point sequences still needs its own follow-up).
- **Route cards no longer mislabel every route as "limited service" during an agency's overnight hours**: a route with no service data for whichever period happened to be selected was treated the same as a genuinely short-window/peak-only route. This wasn't agency-specific — any route without 24/7 service, viewed during its own no-service hours, was affected everywhere, not just on the agency (Mi Transporte) where it was first noticed. Routes with no service in the selected period now correctly show nothing for that period instead of a misleading "limited service" label ([#227](https://github.com/Civic-Minds/Atlas/issues/227)).
...
- **Seattle specialty agencies**: Added Seattle Center Monorail and Snoqualmie Valley Transportation (SVT). Seattle Streetcar is configured but still pending — its feed is King County Metro's full multi-agency GTFS and needs agency-scoped filtering the pipeline doesn't support yet ([#214](https://github.com/Civic-Minds/Atlas/issues/214)), so it won't appear on the map until that's built.
