...
### Added
- **Ouija Backend (`server/`)**: Scaffolded the Project Ouija persistence backend inside the Atlas repo. Node.js/TypeScript/Express server with a Postgres database, continuous GTFS-RT ingestion per agency, and structured ingestion health logging. Polls configured agencies every 30 seconds and writes every observed vehicle position to a `vehicle_positions` table — building the historical record needed for OTP analysis. Agencies now live: DRT (PULSE BRT + N1/N2 overnight), TTC streetcars, MBTA Green Line (B/C/D/E), SEPTA subway-surface trolleys, OC Transpo Transitway BRT, TriMet MAX + FX2 + Streetcar, Metro Transit Arterial/Freeway BRT, MTA NYC SBS, GCRTA HealthLine + rail, Edmonton LRT (Capital/Metro/Valley Lines). San Diego MTS Rapid stub ready to activate once API key arrives.
- **Project Ouija Roadmap (`docs/ROADMAP_NEXTGEN.md`)**: Documents the vision, architecture shift, four development phases, and what already exists vs. what's missing. Referenced from main `ROADMAP.md`.
