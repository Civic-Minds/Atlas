...
- **InfoPanel slide carousel offset**: Simplified the slide container styling to a standard `width: 100%` width with viewport-relative translations (`-100%`, `-200%`) and wrapped the inner flex col slide in a plain block wrapper div, resolving browser layout calculation bugs that shifted flex content and cut off the left edge.
