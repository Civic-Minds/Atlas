...
- **Restored missing YRT trip-lookup data**: `atlas/yrt-trips.json` had silently failed to publish during a past refresh, so York Region Transit's live vehicles had no direction/destination data to fall back on. Re-published via a forced refresh (#230).
- **Mexico coverage hidden from production until validated**: Mi Transporte (Guadalajara) was visible to real users while known data-quality issues in its coverage are still being fixed. Now hidden from the public build (and the external agency directory other tools consume) while remaining visible in local development for QA (#222).
