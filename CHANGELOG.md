...
- **Centralize time periods configurations**: Added `TIME_PERIODS` metadata to `shared/config.ts` defining start/end hours and labels. Integrated it dynamically into `pipeline/process-core.ts` (mapped to minutes), `FilterChips.tsx` (for calculating the active period), and `Corridors.tsx` (automatically formatting the timeline columns and layouts), removing duplicate hardcoded hours and labels.
