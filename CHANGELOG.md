...
### Added
- **Performance Module** (`/performance`): New agency-facing operational dashboard with 5 tabs — **Overview** (health score, match rate, active routes, alert counts, top 5 bottlenecks, route performance table), **Bottlenecks** (segment-level delay analysis from `segment_metrics`, ranked by avg delay delta, with actionable intelligence callouts), **Ghost Buses** (scheduled-vs-observed trip comparison from the ghost detection engine, per-route ghost rate and coverage bars), **Dwell Analysis** (stop-level dwell time ranking from `stop_dwell_metrics`, with boarding bottleneck diagnosis), and **Live Corridors** (real-time corridor headway comparison with bunching detection). All five tabs call existing backend APIs (`/api/intelligence/bottlenecks`, `/api/intelligence/dwells`, `/api/intelligence/ghosts`, `/api/intelligence/matching-stats`, `/api/corridors/performance`) that previously had no frontend. Added to secondary navigation alongside Pulse and Map.
- **API Client: Ghost Buses, Matching Stats, Trends**: Added `fetchGhostBuses()`, `fetchMatchingStats()`, and `fetchTrends()` to `atlasApi.ts` — typed client functions for three server endpoints that were backend-only.
