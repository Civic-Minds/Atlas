...
### Changed
- **History tracking now covers all agencies**: removed the `HISTORY_SLUGS` opt-in set in `refresh.ts` — every agency with a `feedUrl` now writes a headway snapshot to `atlas-archive` on each GTFS update. Snapshot key is `feed_end_date` so only actual service period changes create new snapshots; identical re-downloads are skipped.
- **History auto-discovery in `build-history.ts`**: no longer requires manual opt-in via `BASE_HISTORY` for registered agencies. Combines archive snapshots from `atlas-archive` with current GeoJSON from `atlas` — only 1 archived service period is needed for a comparison (current atlas data provides the "now" data point). Any registered agency where at least one route's headway differs between the archive and current atlas is included automatically. `BASE_HISTORY` is for manual case-study data only (e.g. GCRTA HealthLine pre-2026 snapshots). Region is now read from `index.json` rather than hardcoded, fixing US agencies (Ohio, Michigan, New York). Snapshots now also store `routeLongName`. Routes with no headway change are excluded.
