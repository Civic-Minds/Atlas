...
- **TTC streetcar position archive**: the GTFS-RT archiver Worker now snapshots streetcar positions (id, route, lat/lon, speed, bearing) every 5 minutes to R2 (`positions/ttc/`), 30-day retention. Enables "how slow are streetcars, by route and hour" analysis. Protobuf parser extended to decode floats.
