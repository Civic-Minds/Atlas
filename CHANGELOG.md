...
### Changed
- **Agency list label standard**: muted secondary is **place/sector only** — never an acronym. `Edmonton Transit · ETS` → `ETS`; place rows stay `BC Transit · Kelowna`. Registry name for Edmonton set to full “Edmonton Transit Service (ETS)”.
