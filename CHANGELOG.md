...
- **Lettered route variant grouping**: route cards detect variant families (GRTC-style 1/1A/1B/1C) and show "Runs with" chips to hop between variants plus the combined frequency where they overlap (e.g. four 30-40 min variants → every ~9 min on the shared trunk). Conservative detection: numeric base + single letter, same agency, 2+ members.
