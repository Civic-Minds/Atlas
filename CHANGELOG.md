...
- **History auto-discovery in `build-history.ts`**: no longer requires manual opt-in via `BASE_HISTORY` for registered agencies. After reading R2 snapshots, any agency from `index.json` with ≥2 period snapshots where at least one route changed headway is automatically included. `BASE_HISTORY` now only covers agencies not in the registry (GCRTA case studies). Snapshots now also store `routeLongName`. Routes with no headway change across snapshots are excluded.
