...
- **France expansion: +34 candidate cities (dry-run only, still hidden)**: major metros including Lyon, Marseille, Toulouse, Nice, plus mid-size networks across most régions — all dry-run validated with route geometry, still `hiddenInProduction` / `pmtilesPending` until a deliberate country launch. No R2 writes.
