...
async function poll() {
  ...
  for (const e of feed.entity) {
    ...
    if (prev && prev.directionId === directionId) {
      const template = cfg.scheduleOffsetMin[directionId];
...
async function main() {
  console.log(`Polling Route 1 (route_ids=${cfg.routeIds.join(', ')}) every ${POLL_INTERVAL_MS / 1000}s for ${RUN_MINUTES} min...`);
