...
const nextRecords = catalog.agencies.slice(100, 150)
...
if (nextRecords.length !== 50) throw new Error(`Expected 50 new records, got ${nextRecords.length}`);
audit.auditVersion = 3;
audit.scope = 'First 150 agencies in catalog order; official system-map evidence first, with approved official rider-facing definitions when the map is insufficient.';
audit.records.push(...nextRecords);
fs.writeFileSync(auditPath, `${JSON.stringify(audit, null, 2)}\n`);
console.log(`Extended ${auditPath} to ${audit.records.length} records; added ${nextRecords.length} batch-three records.`);
