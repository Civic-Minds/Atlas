...
export function applyAnalysisCriteria(
    ...
    for (const [, group] of rollupGroups) {
        ...
        const stats = allGaps.length > 0
            ? computeHeadwayStats([...new Set(allTimes)].sort((a, b) => a - b))
            : { avg: 0, median: 0, peakHeadway: 0, peakWindow: { start: 0, end: 0 }, variance: 0, bunchingFactor: 0, reliabilityScore: 0, consistencyScore: 0, bunchingPenalty: 0, outlierPenalty: 0, gaps: [] };
