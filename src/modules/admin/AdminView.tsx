...
export default function AdminView() {
    ...
    const [activeTab, setActiveTab] = useState<'ingest' | 'pulse'>('pulse');
        ...
        <div className="atlas-page max-w-4xl mx-auto py-12 px-6">
            <header className="mb-12 border-b border-[var(--border)] pb-8 flex items-center justify-between">
                <div>
                    ...
                    <h1 className="atlas-h1 mb-2">Platform Console</h1>
                    <p className="text-[var(--text-muted)] text-lg">System-wide intelligence and data management.</p>
                </div>
                <div className="flex items-center gap-4 bg-[var(--item-bg)] p-1 rounded-xl border border-[var(--border)]">
                    <button 
                        onClick={() => setActiveTab('pulse')} 
                        className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pulse' ? 'bg-indigo-500 text-white shadow-lg' : 'text-[var(--text-muted)] hover:text-[var(--fg)]'}`}
                    >
                        Pulse
                    </button>
                    <button 
                        onClick={() => setActiveTab('ingest')} 
                        className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'ingest' ? 'bg-indigo-500 text-white shadow-lg' : 'text-[var(--text-muted)] hover:text-[var(--fg)]'}`}
                    >
                        Ingest
                    </button>
                </div>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'pulse' ? (
                    ...
                    >
                            ...
                            <div className="precision-panel p-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex justify-between items-center py-3 border-b border-[var(--border)]">
                                        ...
                                        <span className="text-sm font-bold text-[var(--fg)]">PostgreSQL // atlas_lab</span>
                                    </div>
                                    <div className="flex justify-between items-center py-3 border-b border-[var(--border)]">
                                        <span className="atlas-label">Sync Mode</span>
                                        <span className="text-sm font-bold text-indigo-500 font-mono">REALTIME_POLL</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Catalog Feeds */}
                        <div className="precision-panel p-8 bg-[var(--item-bg)] border-[var(--border)]">
                            <h2 className="text-xl font-bold mb-6 flex items-center gap-3">
                            ...
                            {feeds.length === 0 ? (
                                <div className="text-center py-12 border border-dashed border-[var(--border)] rounded-2xl space-y-6">
                                    <Database className="w-10 h-10 text-indigo-400 opacity-30 mx-auto" />
                                    <div>
                                        <p className="font-bold text-[var(--fg)] mb-2">Your agency's data is being provisioned</p>
                                        <p className="text-sm text-[var(--text-muted)] max-w-md mx-auto leading-relaxed">
                                            Your schedule and real-time feeds should be configured before your first login.
                                            If you're seeing this, your agency's GTFS data hasn't been loaded yet — reach out to your account administrator.
                                        </p>
                                    </div>
                                    <div className="flex flex-col items-center gap-2 text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">
                                        <span className="text-indigo-400">Switch to the Pulse tab to check real-time feed status</span>
                                    </div>
                                </div>
                            ) : (
