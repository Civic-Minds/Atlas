...
export default function VerifierView() {
    ...
    if (!isAuthenticated) {
        return (
            <ModuleLanding
                title="Audit"
                description="Verify the frequency analysis by comparing it against real agency schedules. Mark each route as correct, wrong, or unsure."
                icon={Shield}
                features={[
                    {
                        title: "Side-by-side review",
                        description: "See the calculated frequency on the left and the agency's published schedule on the right.",
                        icon: <Shield className="w-5 h-5 text-indigo-500" />
                    },
                    {
                        title: "Quick decisions",
                        description: "Mark each route as correct, wrong, or unsure using keyboard shortcuts (Q/W/E).",
                        icon: <CheckCircle2 className="w-5 h-5 text-indigo-500" />
                    },
                    {
                        title: "Headway override",
                        description: "When a frequency is wrong, enter the observed headway from the schedule.",
                        icon: <Settings className="w-5 h-5 text-indigo-500" />
                    },
                    {
                        title: "Export verified data",
                        description: "Download the verified frequency data as JSON when you're done.",
