...
exports.getStaticPool = getStaticPool;
exports.getAgencyFeedVersion = getAgencyFeedVersion;
exports.getRouteScheduleAroundTime = getRouteScheduleAroundTime;
const pg_1 = require("pg");
let staticPool = null;
function getStaticPool() {
    if (!staticPool) {
        staticPool = new pg_1.Pool({
 ...
 */
async function getAgencyFeedVersion(agencySlug) {
    const db = getStaticPool();
    const res = await db.query(`SELECT v.id FROM feed_versions v
     JOIN gtfs_agencies a ON v.gtfs_agency_id = a.id
     WHERE a.agency_slug = $1 AND v.is_current = TRUE`, [agencySlug]);
 ...
 */
async function getRouteScheduleAroundTime(versionId, routeId, timeSeconds) {
    const db = getStaticPool();
    ...
    const res = await db.query(`SELECT
     ...
     ORDER BY st.gtfs_trip_id, st.stop_sequence`, [versionId, routeId, timeMinutes - WINDOW_MINUTES, timeMinutes + WINDOW_MINUTES]);
    const tripMap = new Map();
    for (const row of res.rows) {
        const { tripId, ...stop } = row;
        if (!tripMap.has(tripId))
            tripMap.set(tripId, []);
        tripMap.get(tripId).push(stop);
    }
    return tripMap;
}
