...
exports.getMatchDiagnostics = getMatchDiagnostics;
 ...
 */
function localSecondsFromMidnight(date, timezone) {
 ...
 */
function saveToCache(tripId, data) {
...
function getMatchDiagnostics(agencyId) {
    if (agencyId) {
 ...
 */
async function matchPositions(agency, positions) {
    const diag = {
        agencyId: agency.id,
    ...
    const versionId = await (0, static_db_1.getAgencyFeedVersion)(agency.id);
    if (!versionId) {
    ...
    const db = (0, static_db_1.getStaticPool)();
    ...
    const dbResolvedIds = new Set(); // trip IDs resolved via static DB lookup
    if (missingTripIds.length > 0) {
       ...
       WHERE st.feed_version_id = $1 AND st.gtfs_trip_id = ANY($2)`, [versionId, missingTripIds]);
        ...
        for (const st of res.rows) {
            if (!resultsMap.has(st.tripId))
                resultsMap.set(st.tripId, []);
        ...
        if (stillUnmatched.length > 0) {
            ...
            const obsSeconds = localSecondsFromMidnight(now, (0, config_1.agencyTimezone)(agency.id));
    ...
    for (const id of uniqueTripIds) {
        if (preExistingCachedIds.has(id) || dbResolvedIds.has(id)) {
            ...
            diag.tripIdMismatch++;
            if (diag.sampleUnmatchedTripIds.length < 5)
                diag.sampleUnmatchedTripIds.push(id);
        }
    }
    const segmentMetrics = [];
    const stopDwellMetrics = [];
    // ---------------------------------------------------------------------------
    ...
    if (tripsWithPositions.length > 0) {
        ...
        try {
            ...
            const detourRes = await db.query(`WITH vps AS (
           SELECT 
         ...
         ORDER BY vps.idx`, [versionId, tripIds, lons, lats]);
            // Map results back by index/position
            detourRes.rows.forEach(row => {
                detourMap.set(`${row.trip_id}_${row.distance}`, {
            ...
            tripsWithPositions.forEach((p, idx) => {
                const match = detourRes.rows.find(r => r.idx === (idx + 1));
                if (match) {
    ...
    for (const p of positions) {
        ...
        if (!target) {
            ...
            for (const s of sched) {
                if (s.stopLat === undefined || s.stopLon === undefined)
                    continue;
        ...
        if (target && target.arrivalTime !== null) {
            ...
            const obs = p.observedAt instanceof Date ? p.observedAt : new Date(p.observedAt);
            // GTFS arrival_time is in the agency's local timezone; use that for seconds-from-midnight.
            const observedSeconds = localSecondsFromMidnight(obs, (0, config_1.agencyTimezone)(agency.id));
            ...
            if (lastState && lastState.stopId !== target.stopId) {
                const lastObsAt = lastState.observedAt instanceof Date ? lastState.observedAt : new Date(lastState.observedAt);
            ...
            if (lastState && lastState.stopId === target.stopId && lastState.dwellStart && p.currentStatus !== 1) {
                const dwellStart = lastState.dwellStart instanceof Date ? lastState.dwellStart : new Date(lastState.dwellStart);
