...
exports.aggregateCorridorPerformance = aggregateCorridorPerformance;
const db_1 = require("../storage/db");
const static_db_1 = require("../storage/static-db");
const logger_1 = require("../logger");
async function aggregateCorridorPerformance(agencyId, startTime, endTime, overrideFeedVersionId, bunchingThresholdSeconds = 60) {
    const realtimeDb = (0, db_1.getPool)();
    const staticDb = (0, static_db_1.getStaticPool)();
    logger_1.log.info('Headway', 'Aggregating corridor performance', { agencyId, startTime, endTime, overrideFeedVersionId });
    ...
    if (!feedVersionId) {
        const agencyRow = await staticDb.query(`SELECT fv.id AS feed_version_id
    ...
    const corridors = await staticDb.query(`SELECT link_id, stop_a_id, route_ids, avg_headway as scheduled_headway
     FROM corridor_results
     WHERE feed_version_id = $1 AND day_type = $2
     LIMIT 20`, [feedVersionId, dayType]);
    if (corridors.rows.length === 0)
        return [];
    ...
    const allStopIds = [...new Set(corridors.rows.map((c) => c.stop_a_id))];
    const allArrivalsRes = await realtimeDb.query(`SELECT
       stop_id,
       trip_id,
       route_id,
     ...
     ORDER BY stop_id, arrival_time ASC`, [agencyId, allStopIds, startTime, endTime]);
    // Index arrivals by stop_id for fast in-memory aggregation
    const arrivalsByStop = new Map();
    for (const row of allArrivalsRes.rows) {
        if (!arrivalsByStop.has(row.stop_id))
            arrivalsByStop.set(row.stop_id, []);
