...
exports.detectGhostBuses = detectGhostBuses;
const db_1 = require("../storage/db");
const static_db_1 = require("../storage/static-db");
async function detectGhostBuses(agencyId, windowMinutes = 60) {
    ...
    const agencyRow = await staticDb.query(`SELECT ga.id as gtfs_agency_id, fv.id as feed_version_id
    ...
    const activeServices = await staticDb.query(`SELECT service_id FROM calendar_services
     ...
     WHERE feed_version_id = $1
       AND exception_date = CURRENT_DATE
       AND exception_type = 1`, [feed_version_id]);
    ...
    const scheduledTrips = await staticDb.query(`SELECT
     ...
     GROUP BY t.gtfs_route_id, t.gtfs_trip_id`, [feed_version_id, serviceIds, startMinutes, endMinutes]);
    // 4. Find trips that actually broadcasted positions in the window
    const observedTripsResult = await realtimeDb.query(`SELECT DISTINCT trip_id 
     FROM vehicle_positions 
     WHERE agency_id = $1 
       AND observed_at >= $2 
       AND observed_at <= $3`, [agencyId, startTime, now]);
