...
const importer_1 = require("../import/importer");
...
router.get('/agencies', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    const tenant = req.tenant;
    const params = [];
    const where = tenant?.agencyId ? `WHERE aa.slug = $${params.push(tenant.agencyId)}` : '';
    const result = await (0, static_db_1.getStaticPool)().query(`SELECT
...
router.get('/agencies/:slug/routes', async (req, res) => {
    ...
    const result = await (0, static_db_1.getStaticPool)().query(`SELECT
     ...
     WHERE aa.slug = $1
       ...
       AND rfr.day_type = $2
     ORDER BY rfr.route_short_name, rfr.direction_id`, [req.params.slug, dayType]);
    res.json(result.rows);
});
// GET /api/import/agencies/:slug/simulate/routes
// Returns all routes for the Simulate module (id, name, type, color).
router.get('/agencies/:slug/simulate/routes', auth_1.requireAuth, async (req, res) => {
    const pool = (0, static_db_1.getStaticPool)();
    const fvRow = await pool.query(`SELECT fv.id
     FROM feed_versions fv
     JOIN gtfs_agencies ga ON ga.id = fv.gtfs_agency_id
     JOIN agency_accounts aa ON aa.id = ga.agency_account_id
    ...
    const result = await pool.query(`SELECT gtfs_route_id, route_short_name, route_long_name, route_type, route_color
     FROM routes
     WHERE feed_version_id = $1
     ORDER BY route_short_name`, [feedVersionId]);
...
router.get('/agencies/:slug/simulate/route/:routeId', auth_1.requireAuth, async (req, res) => {
    const pool = (0, static_db_1.getStaticPool)();
    ...
    const feedVersionId = fvRow.rows[0].id;
    // Route metadata
    const routeRow = await pool.query(`SELECT gtfs_route_id, route_short_name, route_long_name, route_color
     FROM routes WHERE feed_version_id = $1 AND gtfs_route_id = $2 LIMIT 1`, [feedVersionId, routeId]);
    ...
    const tripRow = await pool.query(`SELECT t.gtfs_trip_id
     FROM trips t
     JOIN (
    ...
    const stopsResult = await pool.query(`SELECT st.stop_sequence, s.gtfs_stop_id, s.stop_name, s.stop_lat, s.stop_lon
     FROM stop_times st
     ...
     ORDER BY st.stop_sequence`, [feedVersionId, tripId]);
    ...
    const shapeResult = await pool.query(`SELECT ST_AsGeoJSON(geom)::json AS geojson
     FROM route_shapes
    ...
    if (shapeResult.rows.length > 0) {
        ...
        shape = coords.map(([lon, lat]) => [lat, lon]);
    }
    if (shape.length === 0) {
        shape = stops.map(s => [s.lat, s.lng]);
    }
    const ROUTE_COLORS = ['6366F1', '10B981', 'F59E0B', 'EF4444', '8B5CF6', '06B6D4', 'EC4899', '14B8A6', 'F97316', '3B82F6'];
