...
exports.benchmarkCache = void 0;
exports.computeBenchmark = computeBenchmark;
exports.scheduleBenchmarkRefresh = scheduleBenchmarkRefresh;
const headway_1 = require("../intelligence/headway");
const ghosts_1 = require("../intelligence/ghosts");
const health_1 = require("../intelligence/health");
const alerts_1 = require("../intelligence/alerts");
...
router.get('/corridors/performance', auth_1.requireAuth, auth_1.requireTenant, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    const { agency, window, threshold } = req.query;
    if (!agency) {
    ...
    try {
        ...
        const results = await (0, headway_1.aggregateCorridorPerformance)(agency, startTime, endTime, undefined, bunchingThresholdSeconds);
        res.json({
            agency,
            windowMinutes,
...
router.get('/ingestion', async (req, res) => {
    ...
    const result = await (0, db_1.getPool)().query(`SELECT agency_id, polled_at, success, vehicle_count, error_msg, notion_sync_at
     FROM ingestion_log
     ${agency ? 'WHERE agency_id = $1' : ''}
     ORDER BY polled_at DESC
     LIMIT ${agency ? '$2' : '$1'}`, agency ? [agency, limit] : [limit]);
...
router.get('/vehicles', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    const agency = req.query.agency;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    }
    const result = await (0, db_1.getPool)().query(`SELECT DISTINCT ON (vehicle_id)
     ...
     ORDER BY vehicle_id, observed_at DESC`, [agency]);
...
router.get('/positions', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    const { agency, route, from, to } = req.query;
    if (!agency || !route) {
        res.status(400).json({ error: 'agency and route are required' });
        return;
    }
    const result = await (0, db_1.getPool)().query(`SELECT vehicle_id, trip_id, lat, lon, speed, bearing, stop_id, stop_sequence, is_detour, dist_from_shape, observed_at
     FROM vehicle_positions
     WHERE agency_id = $1
       AND route_id  = $2
       AND observed_at >= $3
...
router.get('/screen', async (req, res) => {
    const { agency, maxHeadway, windowStart, windowEnd, dayType, directions } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    }
    ...
    const agencyRow = await pool.query(`SELECT fv.id AS feed_version_id
    ...
    if (bothDirs) {
       ...
       ORDER BY base_headway ASC, route_short_name`, [feedVersionId, day, headway, winStart, winEnd]);
        rows = result.rows;
    }
    else {
        // At least one direction qualifies — return the best-performing direction per route.
        const result = await pool.query(`SELECT DISTINCT ON (gtfs_route_id)
         gtfs_route_id,
         route_short_name,
         route_long_name,
         mode_category,
       ...
       WHERE feed_version_id  = $1
         ...
         AND service_span_start <= $4
         AND service_span_end   >= $5
       ORDER BY gtfs_route_id, base_headway ASC`, [feedVersionId, day, headway, winStart, winEnd]);
        rows = result.rows;
    }
...
router.get('/corridors', async (req, res) => {
    const { agency, minRoutes, maxHeadway, windowStart, windowEnd, dayType } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    }
    ...
    const result = await pool.query(`SELECT
     ...
     LIMIT 500`, [feedVersionId, day, minR, headway]);
    // Also fetch route short names for the route_ids in the results
    const allRouteIds = [...new Set(result.rows.flatMap((r) => r.route_ids))];
    let routeNames = {};
    if (allRouteIds.length > 0) {
        const nameRes = await pool.query(`SELECT DISTINCT ON (gtfs_route_id) gtfs_route_id, route_short_name
       FROM routes
       WHERE feed_version_id = $1 AND gtfs_route_id = ANY($2)`, [feedVersionId, allRouteIds]);
    ...
    const corridors = result.rows.map((r) => ({
        ...r,
        route_short_names: r.route_ids.map((id) => routeNames[id] ?? id),
    }));
...
router.get('/intelligence/trends', auth_1.requireAuth, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    const agency = req.query.agency;
    const result = await (0, db_1.getPool)().query(`SELECT 
     ...
     ORDER BY hour ASC`, agency ? [agency] : []);
    res.json({
        ts: new Date().toISOString(),
        trends: result.rows
    });
});
router.get('/intelligence/matching-stats', auth_1.requireAuth, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    const agency = req.query.agency;
    const result = await (0, db_1.getPool)().query(`SELECT 
       agency_id,
       COUNT(*) as total_obs,
     ...
     ${agency ? 'AND agency_id = $1' : ''}
     GROUP BY agency_id`, agency ? [agency] : []);
    const stats = [];
    for (const row of result.rows) {
        try {
            const health = await (0, health_1.calculateAgencyHealth)(row.agency_id);
            const metrics = {
            ...
            await (0, alerts_1.evaluateThresholds)(row.agency_id, metrics);
            stats.push({ ...row, healthScore: health.compositeScore });
        }
        catch (e) {
            logger_1.log.error('Health', 'Failed to calculate health or alerts for agency', { agencyId: row.agency_id, error: e });
...
router.get('/intelligence/match-diagnostics', auth_1.requireAuth, rate_limit_1.diagnosticsLimiter, (req, res) => {
    const agency = req.query.agency;
    const data = (0, matcher_1.getMatchDiagnostics)(agency);
...
router.get('/intelligence/bottlenecks', auth_1.requireAuth, auth_1.requireTenant, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    ...
    try {
        ...
        const metrics = await (0, db_1.getPool)().query(`SELECT 
         route_id, 
         from_stop_id, 
         to_stop_id, 
         COUNT(*) as obs_count,
       ...
       LIMIT $2`, [agency, resultLimit]);
        ...
        const stopsRes = await staticPool.query(`SELECT gtfs_stop_id as id, stop_name as name, stop_lat as lat, stop_lon as lon FROM stops WHERE gtfs_stop_id = ANY($1)`, [stopIds]);
        const routesRes = await staticPool.query(`SELECT gtfs_route_id as id, route_short_name as name FROM routes WHERE gtfs_route_id = ANY($1)`, [routeIds]);
...
router.get('/intelligence/dwells', auth_1.requireAuth, auth_1.requireTenant, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    const { agency, limit } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    ...
    try {
       ...
       LIMIT $2`, [agency, resultLimit]);
        ...
        const routeIds = [...new Set(metrics.rows.map(m => m.route_id))];
        const stopsRes = await staticPool.query(`SELECT gtfs_stop_id as id, stop_name as name FROM stops WHERE gtfs_stop_id = ANY($1)`, [stopIds]);
        const routesRes = await staticPool.query(`SELECT gtfs_route_id as id, route_short_name as name FROM routes WHERE gtfs_route_id = ANY($1)`, [routeIds]);
...
router.get('/intelligence/ghosts', auth_1.requireAuth, auth_1.requireTenant, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    const { agency, window } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    }
    const windowMinutes = parseInt(window ?? '60', 10);
    try {
        const results = await (0, ghosts_1.detectGhostBuses)(agency, windowMinutes);
        res.json({
            agency,
            windowMinutes,
            ts: new Date().toISOString(),
...
router.get('/alerts/thresholds', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    const user = req.user;
    const tenant = await (0, db_1.getTenantForUser)(user.uid);
    if (!tenant)
        return res.status(403).json({ error: 'Access denied' });
    const accountRes = await (0, static_db_1.getStaticPool)().query('SELECT id FROM agency_accounts WHERE slug = $1', [tenant.agencyId]);
    if (accountRes.rows.length === 0)
        return res.status(404).json({ error: 'Account not found' });
    const accountId = accountRes.rows[0].id;
    const result = await (0, static_db_1.getStaticPool)().query('SELECT * FROM alert_thresholds WHERE agency_account_id = $1 ORDER BY created_at DESC', [accountId]);
    res.json(result.rows);
});
// POST /api/alerts/thresholds
router.post('/alerts/thresholds', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    ...
    const accountId = accountRes.rows[0].id;
    try {
        const result = await (0, static_db_1.getStaticPool)().query(`INSERT INTO alert_thresholds 
             (agency_account_id, target_type, target_id, metric, comparison, value, cooldown_minutes, notion_enabled)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [accountId, target_type, target_id, metric, comparison, value, cooldown_minutes || 60, !!notion_enabled]);
...
router.delete('/alerts/thresholds/:id', auth_1.requireAuth, auth_1.requireTenant, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    const tenant = await (0, db_1.getTenantForUser)(user.uid);
    if (!tenant)
    ...
    if (accountRes.rows.length === 0)
        return res.status(404).json({ error: 'Account not found' });
    const accountId = accountRes.rows[0].id;
    await (0, static_db_1.getStaticPool)().query('DELETE FROM alert_thresholds WHERE id = $1 AND agency_account_id = $2', [id, accountId]);
    res.status(204).send();
});
/**
 ...
 */
router.get('/intelligence/audit-service-change', auth_1.requireAuth, auth_1.requireTenant, rate_limit_1.diagnosticsLimiter, async (req, res) => {
    try {
        const { agency } = req.query;
        if (!agency)
            return res.status(400).json({ error: 'agency is required' });
        const staticDb = (0, static_db_1.getStaticPool)();
        ...
        const resultsA = await (0, headway_1.aggregateCorridorPerformance)(agency, startA, endA, base.id);
        const resultsB = await (0, headway_1.aggregateCorridorPerformance)(agency, startB, endB, head.id);
        res.json({
            agency,
...
router.get('/live/route-health', async (req, res) => {
    const { agency, route } = req.query;
    if (!agency || !route) {
        res.status(400).json({ error: 'agency and route are required' });
        return;
    }
    try {
        const db = (0, db_1.getPool)();
        const tz = (0, config_1.agencyTimezone)(agency);
        // Hourly vehicle counts for the last 7 days
        const hourly = await db.query(`SELECT
       ...
       GROUP BY day, hour
       ORDER BY day, hour`, [agency, route, tz]);
        // Current active vehicle count (last 5 min)
        const current = await db.query(`SELECT COUNT(DISTINCT vehicle_id) AS vehicles
       FROM vehicle_positions
       WHERE agency_id = $1 AND route_id = $2
         AND observed_at > NOW() - INTERVAL '5 minutes'`, [agency, route]);
...
router.get('/live/routes', async (req, res) => {
    ...
    try {
        const db = (0, db_1.getPool)();
        const result = await db.query(`SELECT DISTINCT route_id
       FROM vehicle_positions
       WHERE agency_id = $1 AND observed_at > NOW() - INTERVAL '1 hour'
       ORDER BY route_id`, [agency]);
...
router.get('/live/stops', async (req, res) => {
    const { agency, route } = req.query;
    if (!agency || !route) {
        res.status(400).json({ error: 'agency and route are required' });
        return;
    ...
    try {
        const db = (0, db_1.getPool)();
        const result = await db.query(`SELECT stop_id, COUNT(*) AS visits
       FROM vehicle_positions
       WHERE agency_id = $1
         AND route_id = $2
...
router.get('/live/arrivals', async (req, res) => {
    const { agency, route, stop, minutes } = req.query;
    ...
    try {
        ...
        const gaps = arrivals.map(r => r.gap_mins).filter((g) => g !== null);
...
router.get('/live/gap-distribution', async (req, res) => {
    const { agency, route } = req.query;
    if (!agency || !route) {
        res.status(400).json({ error: 'agency and route are required' });
        return;
    ...
    try {
        ...
        const result = await db.query(`WITH arrivals AS (
         SELECT
           stop_id,
           vehicle_id,
       ...
       FROM bucketed
       GROUP BY bucket
       ORDER BY bucket_min`, [agency, route, tz]);
        // Overall percentiles across all gaps
        const totals = await db.query(`WITH arrivals AS (
...
router.get('/live/network-pulse', async (req, res) => {
    const { agency } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    ...
    try {
        const db = (0, db_1.getPool)();
        const tz = (0, config_1.agencyTimezone)(agency);
        const result = await db.query(`WITH hourly AS (
         SELECT
           route_id,
       ...
       ORDER BY rs.worst_gap DESC NULLS LAST`, [agency, tz]);
        res.json({
            agency,
            ts: new Date().toISOString(),
...
router.get('/live/silent-routes', async (req, res) => {
    const { agency } = req.query;
    if (!agency) {
        res.status(400).json({ error: 'agency is required' });
        return;
    ...
    try {
        ...
        const result = await db.query(`SELECT route_id, last_seen
...
async function computeBenchmark() {
    ...
    `);
        const agencies = result.rows.map(r => {
            ...
            const score = Math.max(0, Math.min(100, Math.round(100 - (bunchingPct * 0.5) - (desertPct * 0.35) - Math.max(0, medianGap - 8) * 1.5)));
            return {
        ...
        logger_1.log.info('Benchmark', 'refreshed', { agencyCount: agencies.length });
    }
    catch (err) {
        logger_1.log.error('Benchmark', 'refresh failed', { err: err.message });
    }
    finally {
        client.release();
    }
